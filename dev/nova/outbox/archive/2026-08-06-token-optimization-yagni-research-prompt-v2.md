...
## Research Context: ECHO Protocol v0.2.0 (Full Protocol)

**This is the authoritative source for all implementation decisions.**

### Agent Roster (9 canonical roles)

| # | Agent | Phase | Responsibility | Current Token Usage |
|---|-------|-------|----------------|---------------------|
| 1 | Orchestrator | ALL | Routes work, enforces protocol, spawns agents | High (system prompt + routing logic) |
| 2 | Detective | RED | Codebase analysis, grep call-graphs, find issues | Medium (read-only, reports findings) |
| 3 | Forge | GREEN | Implementation only — writes code from converged FID | High (code generation) |
| 4 | Verifier | AUDIT | Independent double-audit, run tests, check call-graph | Medium (read-only, reports findings) |
| 5 | Recorder | FID | Create, track, archive FIDs. Update CHANGELOG. | Low (structured output) |
| 6 | Thinker | Planning | Deep sequential reasoning for complex problems | High (sequential thinking) |
| 7 | Scout | Explore | Explores files and code to gather context | Medium (file reading) |
| 8 | Researcher | Research | Web search and documentation lookup | Low (web search) |
| 9 | Scribe | Docs | Session summaries and knowledge capture | Low (structured output) |
...
1. **Layer 1:** Aggressive compaction for old messages
2. **Layer 2:** Summary generation for compacted messages
3. **Layer 3:** Tool output truncation
4. **Layer 4:** Agent-specific context filtering
