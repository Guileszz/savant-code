...
describe('ollama-onboarding', () => {
  ...
  test.serial(
    'selectLocalOllamaModel ignores cloud-backed and embedding entries',
    () => {
      expect(
        selectLocalOllamaModel([
          'cloud-model:cloud',
      ...
      expect(
        selectLocalOllamaModel(['nomic-embed-text:latest']),
      ).toBeUndefined()
    },
  )

  test.serial(
    'applyPersistedDirectProviderSettings restores Ollama config from settings',
    () => {
      saveSettings({
  ...
  test.serial(
    'applyPersistedDirectProviderSettings does nothing when no persisted provider',
    () => {
      applyPersistedDirectProviderSettings()
  ...
  test.serial(
    'applyPersistedDirectProviderSettings does nothing when a backend token is present',
    () => {
      saveSettings({
        directProvider: 'ollama',
        directProviderBaseUrl: 'http://localhost:11434/v1',
  ...
  test.serial(
    'detectOllamaAndConfigureDirectProvider returns early when DIRECT_PROVIDER is already set',
    async () => {
  ...
  test.serial(
    'does not override an explicit non-default model with Ollama detection',
    async () => {
      saveSettings({
  ...
  test.serial(
    'clears persisted Ollama mode when no usable local model remains',
    async () => {
      saveSettings({
        directProvider: 'ollama',
        directProviderBaseUrl: 'http://localhost:11434/v1',
  ...
  test.serial(
    'replaces a gateway preference with a local model during Ollama onboarding',
    async () => {
      saveSettings({
  ...
  test.serial(
    'configures an explicitly selected Ollama model when its URL is missing',
    async () => {
      saveSettings({
